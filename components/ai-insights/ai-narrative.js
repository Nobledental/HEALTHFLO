/* ============================================================
 ...
 - Custom events for summary & insight generation
...
(function () {
  ...
  const CONFIG = {
    ...
    exaggerationFactor: 1.12, // ~12% bias for storytelling
    ...
    container.querySelectorAll('.hf-insight-card').forEach((card) => {
      card.addEventListener('click', () => {
        document.dispatchEvent(
          new CustomEvent('hf:insightClick', {
            detail: {
              category: card.dataset.category,
              text: card.querySelector('.hf-insight-text').innerText,
            },
          })
        );
      });
